...
class TestPreflight:
    ...
    def test_config_generation_ready(self, agent_case):
        ...
        assert json.loads(generated.agent_config_contents).get(agent_case['mcpSettingsKey']), (
...
class TestMcpServerRuns:
    ...
    def test_agent_settings_contains_mcp_config(self, sandbox_case):
        ...
        try:
            ...
            fail_with_result(f'{sandbox_case.config["agent"]} settings file was not valid JSON', result, sandbox_case.history)

        expected_entry = {
            'type': sandbox_case.config['mcpEntryType'],
        ...
        mcp_config = settings.get(sandbox_case.config['mcpSettingsKey'], {})
        if mcp_name not in mcp_config:
            fail_with_result(
                f'Expected {sandbox_case.config["agent"]} settings to include MCP entry "{mcp_name}"',
                result,
                sandbox_case.history,
            )
        ...
        if actual_entry.get('type') != expected_entry['type']:
            fail_with_result(
                f'Expected "{mcp_name}" to be a {expected_entry["type"]} MCP entry',
                result,
                sandbox_case.history,
            )

        if sandbox_case.config['mcpCommandStyle'] == 'array':
            expected_command = [expected_entry['command'], *expected_entry['args']]
            if actual_entry.get('enabled') is not True:
        ...
        if actual_entry.get('command') != expected_entry['command']:
            fail_with_result(
                f'Expected "{mcp_name}" command to be {expected_entry["command"]}',
                result,
                sandbox_case.history,
            )
        if actual_entry.get('args') != expected_entry['args']:
            fail_with_result(
                f'Expected "{mcp_name}" args to be {expected_entry["args"]}',
                result,
                sandbox_case.history,
            )
        if actual_entry.get('env') != {}:
            fail_with_result(f'Expected "{mcp_name}" env to be empty', result, sandbox_case.history)
    ...
    def test_agent_mcp_list_sees_spawned_mcp(self, sandbox_case):
        ...
        if sandbox_case.config['agentMcpListNameOutput'] not in combined:
            fail_with_result(
                f'Expected "{sandbox_case.config["agentMcpListNameOutput"]}" in {sandbox_case.config["agent"]} MCP list output',
                result,
                sandbox_case.history,
            )
        if sandbox_case.config['agentMcpListSpawnedOutput'] not in combined.lower():
            fail_with_result(
                f'Expected "{sandbox_case.config["agentMcpListSpawnedOutput"]}" in {sandbox_case.config["agent"]} MCP list output',
                result,
                sandbox_case.history,
            )

...
if (targets.length === 0) {
  console.log(`No supported OpenShell target for host platform "${process.platform}", skipping download.`);
  process.exit(0);
}
...
(async () => {
  ...
  for (const { platform, arch } of targets) {
    ...
    await downloadOpenshell(version, platform, arch, outputDir, digests);
  }
})();

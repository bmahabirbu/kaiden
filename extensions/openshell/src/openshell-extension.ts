...
export class OpenshellExtension {
  ...
  async activate(): Promise<void> {
    ...
    if (!binaryPath) {
      ...
      if (resourcesPath) {
        const bundledBinaryPath = join(resourcesPath, 'openshell', binaryName);
        if (existsSync(bundledBinaryPath)) {
          version = await this.getVersion(bundledBinaryPath);
          if (version) {
            binaryPath = bundledBinaryPath;
            installationSource = 'extension';
            console.log('[openshell] binary found in bundled resources');
          } else {
            console.warn(`[openshell] bundled binary at ${bundledBinaryPath} failed to report a version`);
          }
        } else {
          console.warn(`[openshell] bundled binary not found at ${bundledBinaryPath}`);
        }
      }
    }
